...
def build_direct_dwelyx_url(
    ...
    *,
    ...
    """Build the final Dwelyx destination URL with standard attribution."""
...
def build_dwelyx_url(
    ...
    *,
    ...
    """Build a one-click tracking URL that records the source before opening Dwelyx."""
    tracking_parts = urlsplit(tracking_app_base_url({"PUBLIC_APP_URL": tracking_base_url}))
    query = {
        "go": "dwelyx",
        "target": dwelyx_base_url({"DWELYX_URL": base_url}),
        "source": _slug(source),
        "medium": _slug(medium),
