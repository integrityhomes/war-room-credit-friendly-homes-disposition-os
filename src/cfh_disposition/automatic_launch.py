...
class AutomationLaunchError(RuntimeError):
    """Raised when the approved campaign cannot be sent to the publishing workflow."""
...
def _property_payload(item: OwnerFinanceProperty) -> dict[str, Any]:
    return {
        ...
        "showing_instructions": item.showing_instructions,
...
def build_automatic_launch_payload(
    property_record: OwnerFinanceProperty,
    package: CampaignPackage,
    links_by_key: Mapping[str, Mapping[str, str]],
    *,
    campaign: str,
    approved_by: str,
    ...
    return {
        ...
        "campaign": campaign,
        "property": _property_payload(property_record),
        "buyer_destination": {
...
def dispatch_automatic_launch(
    ...
    try:
        ...
        raise AutomationLaunchError(
            f"The automatic publishing workflow rejected the campaign (HTTP {exc.code}). {detail}"
        ) from exc
    except (URLError, TimeoutError) as exc:
        raise AutomationLaunchError(
            "The automatic publishing workflow could not be reached. No external channel was marked launched."
        ) from exc
