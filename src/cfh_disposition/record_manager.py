...
def _render_property_manager(storage: Storage) -> None:
    ...
    with st.form("edit_property_form"):
        ...
        bedrooms = right.number_input(
            ...
            min_value=0,
            max_value=20,
            value=selected.bedrooms if selected.bedrooms is not None else 0,
        )
        bathrooms = left.number_input(
            "Bathrooms*",
            min_value=0.0,
            max_value=20.0,
            value=float(selected.bathrooms or 0),
            step=0.5,
        )
        ...
        application_url = st.text_input(
            "Application URL",
            value=str(selected.application_url) if selected.application_url else "",
        )
    ...
    if st.button("Delete Property", type="secondary", disabled=not confirmed):
        try:
            ...
            st.session_state.properties = [
                item for item in st.session_state.properties if item.property_id != selected.property_id
            ]
...
def _render_buyer_manager(storage: Storage) -> None:
    ...
    if not options:
        st.info("No buyers are saved yet.")
        return
    ...
    with st.form("edit_buyer_form"):
        ...
        minimum_bedrooms = left.number_input(
            "Minimum bedrooms",
            min_value=0,
            max_value=20,
            value=selected.minimum_bedrooms if selected.minimum_bedrooms is not None else 0,
        )
        maximum_payment = right.text_input(
            "Maximum monthly payment",
            value=_decimal_text(selected.maximum_monthly_payment),
        )
        available_down = left.text_input(
            "Available down payment",
            value=_decimal_text(selected.available_down_payment),
        )
        move_days = right.number_input(
            "Move timeframe in days",
            min_value=0,
            max_value=3650,
            value=selected.move_timeframe_days if selected.move_timeframe_days is not None else 0,
        )
        preferences = list(CommunicationPreference)
        preference = left.selectbox(
            "Preferred contact",
            preferences,
            index=preferences.index(selected.communication_preference),
            format_func=lambda item: item.value,
        )
            ...
            data.update(
                {
                    ...
                    "maximum_monthly_payment": _decimal(maximum_payment) if maximum_payment.strip() else None,
                    "available_down_payment": _decimal(available_down) if available_down.strip() else None,
    ...
    confirmed = st.checkbox(
        "I understand this buyer will be permanently deleted.",
        key=f"confirm_buyer_{selected.buyer_id}",
    )
...
def render_record_manager(storage: Storage) -> None:
    ...
    with property_tab:
        _render_property_manager(storage)
    with buyer_tab:
        _render_buyer_manager(storage)
