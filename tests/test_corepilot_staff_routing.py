...
def test_real_command_bot_page_routes_selected_task_and_retains_all_links(surfaces):  # noqa: F811
    ...
    data = records()
    backend, _ = surfaces
    backend.data.update({k: v for k, v in data.items() if k != 'team_members'})
    before = copy.deepcopy(backend.data)
    with patch('cfh_disposition.staff_profiles.read_team', lambda client: data['team_members']):
        page = page_for('pages/49_CommandCore_Command_Bot.py')
        page.session_state.corepilot_context = {'task_id': 'example-task'}
        page.run()
        page.text_input(key='corepilot_request').set_value('Who should handle this?')
        page.button(key='FormSubmitter:corepilot_request_form-Ask CorePilot').click().run()
        assert not page.exception
        assert page.session_state.corepilot_context == {
            'task_id': 'example-task', 'deal_id': 'example-deal', 'property_id': 'example-property', 'contact_id': 'example-contact'}
        assert any('Coordinator Example' in e.value for e in page.markdown)
    assert backend.data == before and not backend.denied
