...
def test_launch_readiness_checks_full_critical_chain() -> None:
    ...
    assert "dispatch_worker_state_preservation_posture_included: true" in source
...
def test_launch_readiness_requires_critical_safety_postures() -> None:
    ...
    assert 'service === "commandcore-dispatch-worker"' in source
    assert "health.idempotent_dispatch_processing_enabled === true" in source
    assert "health.approval_state_preservation_enabled === true" in source
    assert "dispatch_worker_state_preservation_not_verified" in source
...
def test_deploy_verifies_auditor_authenticated_chain_and_safety_posture() -> None:
    ...
    assert 'assert data.get("dispatch_worker_state_preservation_posture_included") is True' in deploy
