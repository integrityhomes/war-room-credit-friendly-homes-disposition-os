...
def test_blocks_robots_disallow() -> None:
    ...
    with pytest.raises(PublicCompetitorCollectionError, match="robots policy"):
        collect_public_competitor_page(url="https://example.com/landing", market="Peoria, IL", source_name="Example", session=session)


def test_rejects_non_html() -> None:
    session = FakeSession([FakeResponse(404, "", {}), FakeResponse(200, "%PDF", {"Content-Type": "application/pdf"})])
    with pytest.raises(PublicCompetitorCollectionError, match="not an HTML page"):
        collect_public_competitor_page(url="https://example.com/report.pdf", market="Rockford, IL", source_name="Example", session=session)
