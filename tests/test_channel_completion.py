...
def test_completion_audit_covers_every_registered_channel_once():
    ...
    assert {row.key for row in rows} == {channel.key for channel in CHANNELS}
...
def test_completion_summary_matches_rows():
    ...
    assert summary["built"] == 15
    assert summary["tracked"] == 15
    assert summary["ready_to_use"] == 15
    assert summary["needs_connection_or_manual_step"] == 14
