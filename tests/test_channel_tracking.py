...
def test_build_channel_links_creates_one_unique_link_for_every_channel():
    ...
    assert len(rows) == len(CHANNELS) == 15
    assert [row["Channel key"] for row in rows] == [channel.key for channel in CHANNELS]
    assert len({row["Tracked Dwelyx link"] for row in rows}) == 15
...
def test_scorecard_always_contains_all_15_channels_and_zero_rows():
    ...
    rows = channel_scorecard(
        [
            ...
            event("nextdoor_business_post", campaign="local_launch"),
        ]
    )
    ...
    assert by_key["nextdoor"].clicks == 1
    assert by_key["tiktok"].clicks == 0
    assert by_key["marketplace"].traffic_share == 2 / 4 * 100
...
def test_legacy_medium_aliases_roll_into_the_correct_channel():
    ...
    assert canonical_channel_key("Nextdoor Ads") == "nextdoor"


def test_unmapped_clicks_are_separated_from_the_15_channel_scorecard():
