...
const WORKER_VERSION = "2026-08-27.2";
...
async function runMarketSeoAdapter(queued: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  const campaignPayload = queued.campaign_payload && typeof queued.campaign_payload === "object" && !Array.isArray(queued.campaign_payload)
    ...
    : {};
  const property = campaignPayload.property && typeof campaignPayload.property === "object" && !Array.isArray(campaignPayload.property)
    ? campaignPayload.property as Record<string, unknown>
    : {};
  if (!supabaseUrl || !serviceRoleKey) throw new Error("market_seo_adapter_not_configured");
  const response = await fetch(`${supabaseUrl}/functions/v1/commandcore-market-seo`, {
    method: "POST",
    headers: { authorization: `Bearer ${serviceRoleKey}`, "content-type": "application/json" },
    body: JSON.stringify({ property }),
  });
  if (!response.ok) throw new Error(`market_seo_adapter_failed_${response.status}`);
  const result = await response.json();
  if (!result || typeof result !== "object" || Array.isArray(result)) throw new Error("market_seo_adapter_invalid_response");
  return result as Record<string, unknown>;
}

async function buildWorkOrders(queued: Record<string, unknown>) {
  ...
  for (const item of routes) {
    ...
    if (channelKey === "property_page" && routeName === "automatic_ready") {
      orders.push({ channel_key: channelKey, adapter: "cfh_internal_property_page", state: "confirmed_internal_live", external_action_required: false, external_action_started: false, result_status: "live", note: "Property landing pages are rendered from the approved CFH property record." });
      continue;
    }
    ...
    if (channelKey === "market_seo" && routeName === "automatic_ready") {
      try {
        ...
        orders.push({ channel_key: channelKey, adapter: "cfh_market_seo", state: "confirmed_internal_live", external_action_required: false, external_action_started: false, result_status: "live", seo_record: result.seo_record || null });
      } catch (error) {
        console.error("CommandCore market SEO adapter failed", error);
        orders.push({ channel_key: channelKey, adapter: "cfh_market_seo", state: "failed_safe", external_action_required: false, external_action_started: false, result_status: "failed", note: "Market SEO adapter failed; no external action was started." });
      }
      continue;
    }

    if (routeName === "approval_queue") {
      orders.push({ channel_key: channelKey, adapter: "approval_gate", state: "awaiting_approval", external_action_required: true, external_action_started: false, result_status: "awaiting_approval" });
      continue;
    }
    if (routeName === "manual_final_post") {
      orders.push({ channel_key: channelKey, adapter: "manual_final_post", state: "package_ready_for_human", external_action_required: true, external_action_started: false, result_status: "ready" });
      continue;
    }
    if (routeName === "blocked") {
      orders.push({ channel_key: channelKey, adapter: "safety_hold", state: "blocked", external_action_required: false, external_action_started: false, result_status: "blocked" });
      continue;
    }
    orders.push({ channel_key: channelKey, adapter: "unassigned", state: "review_required", external_action_required: false, external_action_started: false, result_status: "review" });
  }
  return orders;
}

Deno.serve(async (req) => {
  if (req.method === "GET") return jsonResponse(200, { ok: true, service: "commandcore-dispatch-worker", version: WORKER_VERSION, status: "healthy", external_execution_enabled: false, market_seo_adapter_enabled: true });
  ...
  try {
    ...
    const workOrders = await buildWorkOrders(queued);
    const updated = { ...queued, status: "adapter_work_prepared", worker_version: WORKER_VERSION, worker_processed_at: new Date().toISOString(), work_orders: workOrders, external_action_started: false };
    ...
    return jsonResponse(200, {
      ...
      channel_results: workOrders.map((item) => ({ channel_key: item.channel_key, status: item.result_status })),
